 ...
 * @author Dima
 */
public class editNickName extends HttpServlet {

    @EJB
    private EJBAccount daoAccount;
        ...
        try (PrintWriter out = response.getWriter()) {
            if (prepare(request, response)) {
                daoAccount
                        .editUserName(
                                request.getHeader(
                                        Constants.ServletConstant.IMEI),
                                request.getParameter(
                                        Constants.ParameterRequestConstant.USER_NAME));
                out.println(new Gson().toJson(daoAccount.getGeneralAccount(request.getHeader(Constants.ServletConstant.IMEI))));
            }
        }
    ...
    private boolean prepare(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            validateRequest.validateRequestParameters(request, response, Constants.ReqestLink.ACCOUNT_EDIT_USERNAME);
            return true;
